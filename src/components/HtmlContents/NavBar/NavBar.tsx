...
import { iconColorState, pageNumState, showLogoState } from "store/atoms";
...
const NavBar = () => {
  ...
  useFrame(() => {
    ...
    if (scroll.range(0, 1 / pageNum) < 1) {
      for (let i = 0; i < 3; i++) {
        colorRef.current.children[i].style.filter =
          width >= 1080
            ? `brightness(${scroll.range(0, 1 / pageNum) * 100}%)`
            : "brightness(0) invert(1)";
      }

      logoRef.current.style.opacity = 0;
      logoRef.current.style.filter = `brightness(${
        scroll.range(0, 1 / pageNum) * 100
      }%)`;
      ...
      for (let i = 0; i < 3; i++) {
        colorRef.current.children[i].style.filter = "brightness(0) invert(1)";
      }
    }
  });
  ...
  return (
    ...
    >
      <img
        ref={logoRef}
        className={`w-[clamp(86.44px,24.011vw,123.49px)] pad:w-[clamp(123.49px,11.434vw,185.23px)] desktop:w-[clamp(185.23px,9.647vw,100vw)] transition duration-[100ms]`}
        src={logo}
        alt="비어갱 로고"
        loading="lazy"
      />
      <div
        ref={colorRef}
        className="flex justify-center items-center gap-x-[clamp(6px,1.667vw,29.56px)] pad:gap-x-[clamp(29.56px,2.737vw,43px)] desktop:gap-x-[clamp(43px,2.240vw,100vw)]"
      >
